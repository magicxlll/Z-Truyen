...
| **4** | **Cú pháp tên chương sạch** | Chuẩn hóa định dạng `Chương {order}_{tên chương}` (ví dụ: `Chương 1_Tiết tử`), loại bỏ tên truyện/tác giả làm hậu tố để màn hình e-ink hiển thị sắc nét, không bị tràn chữ `...`. |
| **5** | **Quản lý tiến trình nền Android** | Kích hoạt `Acquire wakelock` trên Termux và đặt Battery sang `Unrestricted` để Android không đóng băng tiến trình CPU khi tắt màn hình. |
